#!/usr/bin/env bash
# ayir — turn an AppImage into a pacman -bin package.
#
# One tool, five subcommands, each doing exactly one job:
#
#   ayir setup   <path-to.AppImage>        move it in, extract it, report deps
#   ayir deps    <name>                     re-run the dependency check only
#   ayir convert <name> [pkgver] [pkgrel]  build the payload tarball + PKGBUILD
#   ayir build   <name>                     makepkg -f
#   ayir install <name>                     pacman -U the built package
#
...
# path (one directory up from wherever ayir.sh itself lives), so it
...
# Auto-generated by ayir setup — re-running setup overwrites this file,
